...
public: 
    ...
    void error(std::string s) {
        throw ("near line " + std::to_string(lex->line) + ": " + s);
    }
    void match(int t) {
        if ((int)look->tag == t) move(); 
...
Stmt * Parser::stmts() {
    if ((int)look->tag == '}') return Stmt::_null;
...
Expr *Parser::factor() {
    ...
    switch((int)look->tag) {
        ...
        case (int)Tag::NUM:
            x = new Constant(look, Type::_int);      move(); return x;
        case (int)Tag::REAL:
            x = new Constant(look, Type::_float);    move(); return x;
        case (int)Tag::TRUE:{
            x = Constant::_true;     
            bool test = x->type == Type::_bool;
            move(); return x;
       } case (int)Tag::FALSE:
            x = Constant::_false;                    move(); return x;
        case (int)Tag::ID: {
            s = look->toString();
            Id *id = top->get(look);
            if (id == nullptr)
